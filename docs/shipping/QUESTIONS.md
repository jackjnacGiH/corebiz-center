...
## สถานะบัญชีที่ตรวจ 10 กันยายน 2026

- Merchant UAT มีอยู่แล้ว ไม่ต้องสร้างซ้ำและไม่เผยแพร่รหัสในเอกสาร
- Merchant UAT ผูก 11 carrier และ check-price ล่าสุดส่งราคา 11 บริการกลับมาโดยไม่บันทึกร่าง
- CoreBiz UAT ตั้ง billing mode เป็น `prepaid` และบันทึกเบอร์/อีเมลผู้ส่งที่ API ต้องใช้แล้ว; Wallet Verified/Ready แต่ยอด 0.00 ส่วน Credit ยัง Waiting for document/Processing
- COD account แสดง Active แต่ carrier mapping และ bank mapping เป็น 0; COD-01/02 ยังคงเป็น P0
- mutations, webhook และ pickup ยังปิด ข้อ API-02, HOOK-01/02 และ PICKUP-01 ยังไม่ได้รับคำตอบ
- ขั้นตอนและผู้รับผิดชอบอยู่ใน [Checklist เปิดใช้งานจริง](GO_LIVE_CHECKLIST.md)
...
| --- | --- | --- |
| Boss jack / ผู้ดูแลบัญชี | billing mode, merchant ที่จะใช้, ผู้ประสานงาน provider และเอกสารบัญชีฉบับที่มีผลจริง | UAT ใช้ Merchant ปัจจุบันและ prepaid แล้ว; ผู้ประสานงาน/เอกสารฉบับจริงยังรอ ไม่ส่ง credential ผ่าน Git |
| คลัง/ทีมส่ง | จุดรับพัสดุ ชื่อผู้ติดต่อ เบอร์ อีเมล เวลาทำการ และ provider warehouse mapping | บันทึกเบอร์/อีเมลผู้ส่งสำหรับ shipment แล้ว; เวลาทำการและ warehouse mapping ยังรอ |
...
ก่อนเติมเครดิต ให้ขอ Boss jack ยืนยัน environment และจำนวนเงินสุดท้าย แม้ Wallet จะแสดง Verified/Ready แล้ว การอนุญาตให้ตั้งค่าระบบไม่ถือเป็นการอนุมัติธุรกรรมการเงินล่วงหน้า
