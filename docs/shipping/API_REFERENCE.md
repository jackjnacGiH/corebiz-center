...
หน้าเว็บเรียก `POST /functions/v1/shipping-api` พร้อม Supabase access token และส่ง `action` ใน JSON body Backend ตรวจสถานะผู้ใช้และสิทธิ์ขนส่งใหม่ทุกคำขอ ไม่รับ role หรือบัญชี COD จากความเชื่อถือของ browser

| action | ผลลัพธ์หลัก | หมายเหตุ |
| --- | --- | --- |
| `bootstrap` | การตั้งค่า แบรนด์ บัญชี COD และสถานะความพร้อม | ไม่ส่ง provider secret กลับ browser |
| `list` / `get` | รายการส่งและ audit ล่าสุด | หน้าละ 25; ค้นชื่อ/บริษัทผู้รับ เบอร์โทร ที่อยู่ เลขอ้างอิง เลขบิล และ Tracking; ชื่อบริษัทจากลูกค้าในบิลใช้เสริมการแสดงผลของร่างเก่า |
| `compare_rates` | ตารางราคาเรียงจากต่ำไปสูง พร้อมราคาแต่ละกล่อง | รับ `draft` ที่ยังไม่บันทึกได้ ไม่บังคับ carrier; โหลดบริการจากบัญชี API แล้วรวมราคาเฉพาะรายที่ให้ราคาครบทุกกล่อง |
| `order_options` / `order_draft` | คำสั่งซื้อ ผู้รับ และสินค้าในบิล | แปลงน้ำหนักสินค้า kg เป็น g เพื่อช่วยกรอก; พนักงานต้องตรวจน้ำหนักกล่องจริง |
| `recipient_options` | ผู้รับจาก customer และประวัติส่งล่าสุด | รับ `search` อย่างน้อย 3 ตัวอักษร คืนสูงสุด 30 รายการและตัดข้อมูลซ้ำ |
...
รายการส่งที่สร้างเองเพิ่มสินค้าได้ 1–5 แถว เมื่อพิมพ์รหัสตรงกับสินค้าในคลัง ระบบเติมชื่อและน้ำหนักที่มีบันทึกไว้ให้อัตโนมัติ; ถ้าคลังมีน้ำหนักเป็นศูนย์ พนักงานต้องชั่งและกรอกเอง ช่องราคาถูกซ่อนจากฟอร์มและผลค้นหา ส่วนรายการจากคำสั่งซื้อยังดึงรายการตามบิลโดยอัตโนมัติ
