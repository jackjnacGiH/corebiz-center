...
const THAI_SEARCH_PREFIX_RE = /^(?:(?:ผม|ฉัน|หนู|เรา|ลูกค้า)\s*)?(?:(?:สนใจ(?:ซื้อ)?|ต้องการ|อยาก(?:ได้|หา|ทราบ(?:ราคา)?|รู้(?:ราคา)?)|กำลังหา|เอา(?=\s|\d)|ขอเปลี่ยน(?:สินค้า)?(?:เป็น|ไป(?:หา)?)|เปลี่ยน(?:สินค้า)?(?:เป็น|ไป(?:หา)?)|ขอ(?:เป็น|เอา)|ขอ(?:ซื้อ|ราคา|รายละเอียด|สเป[กค]|spec)?|สอบถาม(?:ราคา|รายละเอียด)?|รบกวน(?:ช่วย)?(?:หา|เช็ก|เช็ค|ตรวจสอบ)?|ช่วย(?:หา|เช็ก|เช็ค|ตรวจสอบ)?|หา|เช็ก|เช็ค|ดู|สินค้า)\s*)+/iu;
...
const THAI_AVAILABILITY_PREFIX_RE = /^มี(?:สินค้า)?(?=\s|[A-Z]{2,6}[\s._/-]*\d{2,}|กระดาษทราย|ผ้าทราย|จานทราย|ม้วน|ใบ(?:ขัด|ตัด|เจียร)|ล้อ|ลูกขัด|หินเจียร|แผ่นขัด|แปรง|สายพาน|สว่านลม|เครื่องมือ)\s*/iu;
