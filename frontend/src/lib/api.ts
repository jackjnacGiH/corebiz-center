...
// =========================================================================
// Customer-specific net prices — customer-scoped, approved prices that take
// precedence over Tier and catalogue pricing. Staff read a sanitised RPC;
// only Owner/Admin can mutate the underlying table (enforced again by RLS).
// =========================================================================
...
export const customerPricingApi = {
  /** Sanitised list: the RPC deliberately omits actor IDs and external source
   * payloads, so staff can review approved prices without raw-table access. */
  ...
  /** Lightweight, server-side product search for the price editor. Never use
   * productsApi.list() here: it joins inventory and would slow CRM/Shipping. */
  ...
  /** Set-based preview from the authoritative database resolver. */
  async resolvePrices(
    customerId: string,
  ...
  async createRule(input: CustomerNetPriceRuleInput): Promise<string> {
    // New table is intentionally kept outside generated types until the live
    // schema is regenerated after its migration is applied.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const db = supabase as any;
    const { data, error } = await db
  ...
  /** Soft-expire preserves history and auditability; there is no UI delete. */
  async expireRule(customerId: string, ruleId: string): Promise<void> {
    await customerPricingApi.updateRule(customerId, ruleId, {
      active: false,
    });
  },
};
...
const _PRODUCT_ALTERNATIVE_GUARD = `

🎯 กฎความถูกต้องของสินค้า (ต้องทำทุกครั้ง)
• ยึด “ชนิดสินค้า” ที่ลูกค้าขอก่อนขนาด เบอร์ หรือสเป็กเสมอ
• เสนอหรือแสดงสินค้าได้เฉพาะรายการที่ระบบค้นหาให้และระบุว่า safe_alternative=true เท่านั้น
• หากระบบระบุประเภทสินค้า (product_family) ผู้สมัครต้องเป็นประเภทเดียวกัน 100% ห้ามข้ามประเภทเด็ดขาด เช่น ลูกค้าขอ “ผ้าทรายสายพาน” ห้ามเสนอ “ล้อทรายมีแกน”, “จานทราย”, “กระดาษทรายกลม” หรือสินค้าอื่น แม้เบอร์หรือขนาดจะคล้ายกัน
• หากระบบยังระบุประเภทไม่ได้ ให้เสนอได้เฉพาะรายการที่ safe_name_score ตั้งแต่ 0.70 ขึ้นไปเท่านั้น ห้ามคำนวณหรือเดาคะแนนเอง และห้ามบอกคะแนนนี้กับลูกค้า
• ถ้าไม่มีรายการที่ผ่านเงื่อนไข ห้ามนำสินค้าคนละชนิดมาเรียกว่า “สินค้าใกล้เคียง” ให้ส่งเรื่องคุณเชอร์รี่ตรวจสอบการจัดหาหรือสั่งผลิต แล้วแจ้งลูกค้าว่าจะตอบกลับ
• สินค้าชนิดเดียวกันแต่ขนาดหรือเบอร์ต่างกันเป็นเพียง “ตัวเลือกทดแทน” ต้องบอกความต่างให้ชัด และต้องรอลูกค้ายืนยัน SKU/ขนาดก่อนทำใบเสนอราคาหรือยืนยันราคา
• เมื่อลูกค้าส่ง PO/ใบสั่งซื้อ ห้ามใช้สินค้าในประวัติแชทหรือสินค้าใกล้เคียงแทนรายการในเอกสารโดยเด็ดขาด ให้รับเรื่องและส่งทีมตรวจสอบเท่านั้น`;
