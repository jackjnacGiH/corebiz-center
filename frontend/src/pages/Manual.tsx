...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'ul', items: [
        ...
        'เครื่องมือ: อิโมจิ, สติกเกอร์ (เหมือน LINE — เอาเมาส์ชี้เพื่อดูตัวอย่างใหญ่ก่อนส่ง), แนบรูป, ครอปภาพ, ข้อความสำเร็จรูป (Quick reply), การ์ดสินค้า',
        'ลิงก์ในข้อความ (http/https) กดเปิดได้เลยทั้งฝั่งแอดมินและฝั่งลูกค้า',
      ] },
      { t: 'h', text: 'ใครเป็นคนตอบ' },
      { t: 'ul', items: [
        'เมื่อแอดมินพิมพ์ตอบ ระบบจะเซ็นชื่อผู้ตอบไว้ — ทั้งลูกค้าและเจ้าของเห็นว่าใครเป็นคนตอบ (เช่น "เชอร์รี่: …")',
      ] },
      { t: 'h', text: 'ไฟล์ & รูปจากลูกค้า' },
      { t: 'ul', items: [
        'รูป (สลิป/สินค้า) ที่ลูกค้าส่งจะแสดงในแชท — คลิกเปิดเต็มจอ',
        'ไฟล์เอกสาร เช่น PDF / ใบสั่งซื้อ ที่ลูกค้าส่งมา จะแสดงเป็นการ์ดไฟล์ — กดเปิด/ดาวน์โหลดได้',
        'เอาเมาส์ชี้รูป → ปุ่ม ⬇️ ดาวน์โหลดไฟล์จริงได้เลย',
      ] },
      { t: 'h', text: 'ฟังก์ชันใหม่ & การส่งเอกสารหาลูกค้า' },
      { t: 'ul', items: [
        'การตอบกลับแบบอ้างอิงข้อความ (Quoted Reply): เอาเมาส์ชี้ที่ข้อความของลูกค้า → คลิกปุ่ม "ตอบกลับ" เพื่อพิมพ์ตอบอ้างอิงข้อความเดิม (ลูกค้าฝั่ง LINE จะเห็นเป็นกล่องอ้างอิงสวยงาม)',
        'การส่งไฟล์เอกสาร (PDF/Docs) หา LINE: เนื่องจาก LINE ไม่รองรับการรับส่งไฟล์ตรงๆ ระบบจะทำการอัปโหลดขึ้นคลังและแปลงเป็น "ลิงก์เว็บ" ส่งให้ลูกค้ากดเปิดดูได้โดยอัตโนมัติ',
        'การแชร์พิกัด (LINE Location): พิกัดที่ลูกค้าส่งจะแสดงเป็นที่อยู่พร้อมลิงก์เปิดใน Google Maps ได้ทันที (แอดมินคัดลอกส่งขนส่งได้สะดวกรวดเร็ว)',
      ] },
      { t: 'h', text: 'ผูกแชทกับลูกค้าในระบบ (CRM)' },
      { t: 'p', text: 'ถ้าลูกค้าทักผ่าน LINE โดยไม่ได้ลงทะเบียนเว็บ แอดมินผูกห้องแชทเข้ากับลูกค้าใน CRM เองได้ ที่กล่อง "ลูกค้าในระบบ" (แผงข้อมูลด้านขวา)' },
      { t: 'steps', items: [
        'กดที่กล่อง "ลูกค้าในระบบ" → ค้นชื่อ/รหัส/เบอร์/เลขภาษี → เลือกลูกค้า',
        'เมื่อผูกแล้ว แชทจะโยงเข้าโปรไฟล์ลูกค้า และใบเสนอราคาที่บอทสร้างจะ "ดึงชื่อ/ที่อยู่/เลขภาษีลูกค้า" มาใส่ให้อัตโนมัติ',
      ] },
      { t: 'h', text: 'โน้ตลูกค้า' },
      { t: 'ul', items: [
      ...
      { t: 'h', text: 'ความจำของบอทในแต่ละห้อง' },
      { t: 'ul', items: [
        'กด "ความจำของบอทในห้องนี้" ที่แผงข้อมูลด้านขวา เพื่อดูสิ่งที่บอทจำจากบทสนทนาปัจจุบัน ความสนใจล่าสุด และประวัติที่ยังเกี่ยวข้อง',
        'ความจำแยกตามห้องและลูกค้า ไม่ใช้ข้อมูลข้ามคน และไม่เก็บเบอร์โทร อีเมล ที่อยู่ เลขภาษี ราคา หรือต้นทุนไว้ในข้อความสรุป',
        'Owner/Admin ล็อกข้อมูลสำคัญหรือใส่หมายเหตุให้บอทได้ ข้อมูลที่ล็อกจะไม่ถูก AI เขียนทับ',
      ] },
    ],
  },
  {
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'h', text: 'แท็บ "ข้อเสนอแนะ" — รายงาน AI ประจำเดือน' },
      { t: 'p', text: 'แท็บ "ข้อเสนอแนะ" (ท้ายสุด) ในหน้า AI Agent — ทุก 30 วัน AI จะสรุปพฤติกรรมลูกค้าและการดำเนินงานทั้งหมด แล้วเสนอข้อปรับปรุงเรียงตามลำดับความสำคัญ เพื่อให้เจ้าของตัดสินใจว่าจะพัฒนา/เพิ่มส่วนไหนต่อ' },
      { t: 'ul', items: [
        'วิเคราะห์: ลูกค้าใหม่/ระดับลูกค้า, ใบเสนอราคา+มูลค่า, สินค้าที่ถูกขอราคามากสุด, ปริมาณแชทและการตอบของบอท/พนักงาน, สต็อกต่ำ/หมด, คะแนนความพึงพอใจ',
        'ข้อเสนอแนะแต่ละข้อมี: ลำดับความสำคัญ (1=สูงสุด), ด้าน (ลูกค้า/การขาย/แชท/สต็อก/การตลาด/ระบบ) และระดับความยาก (เล็ก/กลาง/ใหญ่)',
      ] },
      { t: 'steps', items: [
        'ระบบสร้างรายงานให้อัตโนมัติทุกวันที่ 1 ของเดือน (ดูในแท็บนี้ ไม่ส่งเข้า LINE)',
        'กด "สร้างรายงานเดี๋ยวนี้" เพื่อให้ AI วิเคราะห์รอบล่าสุดทันที',
        'กดปุ่มวันที่ใต้รายงานเพื่อดูรายงานย้อนหลัง',
      ] },
      { t: 'note', text: 'AI ทำหน้าที่วิเคราะห์และ "เสนอ" เท่านั้น — ไม่แก้ไขหรือ deploy ระบบเอง การตัดสินใจและการลงมือพัฒนาเป็นของเจ้าของเสมอ' },
    ],
  },
  {
    id: 'users',
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'h', text: 'FlowAccount (อ่านประวัติราคา)' },
      { t: 'steps', items: [
        'เปิดแท็บการเชื่อมต่อ → กด "เชื่อมต่อ FlowAccount" แล้วอนุญาตบริษัท J NAC ที่กำหนดไว้',
        'กด "ซิงก์ราคา" เพื่อนำเฉพาะประวัติราคา 6 เดือนของลูกค้า LINE ที่ยืนยันแล้วและทักล่าสุด สูงสุด 100 ราย มาเก็บในแคชส่วนตัว',
        'ตรวจสถานะซิงก์สำเร็จก่อนเปิด "ความจำลูกค้าแบบมีโครงสร้าง" ให้บอทใช้ข้อมูลใหม่',
      ] },
      { t: 'note', text: 'การเชื่อมนี้อ่านข้อมูลเท่านั้น ไม่สร้าง แก้ไข หรือลบเอกสารใน FlowAccount และไม่เพิ่มแพ็กเกจค่าใช้จ่าย' },
      { t: 'h', text: 'อื่นๆ' },
      { t: 'ul', items: [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'ul', items: [
        ...
        'สินค้าหมด/ไม่ตรง → เสนอสินค้าใกล้เคียง หรือเสนอสั่งผลิต (ถ้าลูกค้าส่งใบสั่งซื้อ/เอกสารสั่งซื้อมา จะไม่เสนอสินค้าใกล้เคียงแทรก)',
        'ขอใบเสนอราคา (ระบุสินค้า+จำนวน) → บอทสร้างใบเสนอราคาฉบับร่างจริง + ส่ง "ลิงก์ใบเสนอราคา" ให้ลูกค้าเปิดดู/ดาวน์โหลด PDF ได้ทันที (ไม่ต้องล็อกอิน — ดูหัวข้อถัดไป)',
        'อ่านรูปได้ (vision) — ดูสลิป/รูปสินค้าแล้วเข้าใจ',
        'ถ้าสินค้ามีหลายขนาด เบอร์ หรือรูปแบบ บอทจะถามเฉพาะข้อมูลที่ยังขาดก่อนค้นซ้ำ แล้วส่งต่อพนักงานเมื่อหาให้ตรงไม่ได้จริง',
        'จำบริบทแยกตามลูกค้าและห้องแชต เพื่อคุยต่อจากเรื่องล่าสุดโดยไม่ถามข้อมูลเดิมซ้ำ',
        'เลือกราคาทีละสินค้า: ราคาเน็ตเฉพาะลูกค้า → ประวัติราคาที่ใช้ได้จาก FlowAccount → ราคา Tier ที่คำนวณจากราคาปกติหลังส่วนลด → ราคาปกติ',
        'ไม่เปิดเผยราคาทุน/ต้นทุน เด็ดขาด',
      ] },
      { t: 'note', text: 'FlowAccount ใช้เป็นแหล่งอ่านประวัติราคาย้อนหลัง 6 เดือนเท่านั้น ใบเสนอราคายังคงสร้างใน CoreBiz และเจ้าหน้าที่นำไปจัดการใน FlowAccount เอง' },
  ...
  },
  {
    id: 'quotelink',
    icon: '🧾',
    title: 'ลิงก์ใบเสนอราคา (ส่งให้ลูกค้าเปิดเอง ไม่ต้องล็อกอิน)',
    blocks: [
      { t: 'p', text: 'เมื่อบอท (หรือระบบ) สร้างใบเสนอราคาจากแชท ลูกค้าที่ไม่ได้ลงทะเบียนผ่านเว็บก็เปิดดูได้ทันที — ระบบส่ง "ลิงก์สาธารณะ" ของใบเสนอราคานั้นเข้าไปในแชทให้เลย' },
      { t: 'h', text: 'ลูกค้าได้อะไร' },
      { t: 'ul', items: [
        'ข้อความในแชทพร้อมลิงก์ เช่น https://www.jnac.online/center/q/<รหัสลับ> — กดเปิดได้เลย ไม่ต้องล็อกอิน',
        'หน้าใบเสนอราคาหน้าตาเหมือนเอกสารในระบบเป๊ะ (โลโก้ + ชื่อบริษัท JNAC + ข้อมูลลูกค้า + รายการ + ยอดสุทธิ)',
        'มีปุ่ม "ดาวน์โหลด / พิมพ์ PDF" — บันทึกเป็นไฟล์ PDF ได้',
      ] },
      { t: 'h', text: 'ชื่อ/ที่อยู่ลูกค้าขึ้นอัตโนมัติ' },
      { t: 'ul', items: [
        'ถ้าห้องแชทผูกกับลูกค้าใน CRM แล้ว (LINE ที่ยืนยันตัวตน หรือแอดมินผูกเองที่กล่อง "ลูกค้าในระบบ") ใบเสนอราคาจะดึงชื่อ/ที่อยู่/เลขภาษีลูกค้ามาใส่ให้เอง',
        'ถ้ายังไม่ได้ผูก → ใบจะยังไม่มีชื่อลูกค้า ให้แอดมินผูกลูกค้าในแชท แล้วใบที่สร้างครั้งถัดไปจะเติมให้อัตโนมัติ (หรือเลือกลูกค้าในหน้าใบเสนอราคาเองก็ได้)',
      ] },
      { t: 'h', text: 'ข้อมูลบริษัท + โลโก้บนเอกสาร' },
      { t: 'p', text: 'หัวเอกสาร (ชื่อบริษัท, ที่อยู่, เลขภาษี, เบอร์โทร, โลโก้) ดึงจาก "ข้อมูลบริษัท" ในหน้า ตั้งค่าระบบ → แก้ที่นั่นแล้วเอกสาร/ลิงก์ใบเสนอราคาจะอัปเดตตามทันที' },
      { t: 'note', text: 'ลิงก์ใช้ "รหัสลับ" เฉพาะใบนั้น เดาไม่ได้ — เปิดดูได้เฉพาะคนที่มีลิงก์ (รูปแบบเดียวกับลิงก์แบบสอบถามความพึงพอใจ)' },
    ],
  },
  {
    id: 'notify',
